            ...
            with open(downloadPath, 'wb') as file:
                while recvSize != fileSize:
                    if (fileSize-recvSize)>1024:
                        data_block=conn.recv(1024)
                        file.write(data_block)
                        recvSize+=len(data_block)
                    else:
                        data_block=conn.recv(fileSize-recvSize)
                        file.write(data_block)
                        recvSize=fileSize
                        break
...
def video_demo(conn,addr):
    path=conn.recv(10000).decode()
    print("[*]Get image path : " + str(path))
    return str(path)
    #print(result)
